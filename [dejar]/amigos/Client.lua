...
GUIEditor.window[1] = guiCreateWindow(screenW - 414 - 10, (screenH - 391) / 2, 414, 391, "~[Sistema de chat privado]~", false)
...
guiGridListAddColumn(GUIEditor.gridlist[1], "Jugador", 0.7)
guiGridListAddColumn(GUIEditor.gridlist[1], "Estado", 0.2)
GUIEditor.button[1] = guiCreateButton(274, 144, 130, 22, "Iniciar un chat", false, GUIEditor.window[1])
guiSetProperty(GUIEditor.button[1], "NormalTextColour", "FF1FC100")
GUIEditor.button[2] = guiCreateButton(274, 176, 129, 22, "Bloquear jugador", false, GUIEditor.window[1])
guiSetProperty(GUIEditor.button[2], "NormalTextColour", "FFD00004")
GUIEditor.button[3] = guiCreateButton(275, 208, 128, 22, "Darle un aviso al jugador", false, GUIEditor.window[1])
guiSetProperty(GUIEditor.button[3], "NormalTextColour", "FFDE6E00")
GUIEditor.button[4] = guiCreateButton(10, 359, 254, 22, "Finalizar el chat", false, GUIEditor.window[1])
...
GUIEditor.label[1] = guiCreateLabel(10, 22, 254, 15, ".• Buscar jugadores •.", false, GUIEditor.window[1])
guiSetFont(GUIEditor.label[1], "default-bold-small")
guiLabelSetHorizontalAlign(GUIEditor.label[1], "center", false)
guiLabelSetVerticalAlign(GUIEditor.label[1], "center")
GUIEditor.combobox[1] = guiCreateComboBox(273, 75, 131, 64, "Seleccionar estado", false, GUIEditor.window[1])
GUIEditor.label[2] = guiCreateLabel(273, 50, 131, 15, ".• Filtro de jugadores •.", false, GUIEditor.window[1])
guiSetFont(GUIEditor.label[2], "default-bold-small")
guiLabelSetHorizontalAlign(GUIEditor.label[2], "center", false)
guiLabelSetVerticalAlign(GUIEditor.label[2], "center")
GUIEditor.checkbox[1] = guiCreateCheckBox(287, 240, 116, 16, "No me webees", false, false, GUIEditor.window[1])
...
chat_Windows[player].SendButton = guiCreateButton(421, 280, 104, 24, "Enviar un mensaje", false, chat_Windows[player].window)
chat_Windows[player].Xclose = guiCreateButton(582, 278, 37, 26, "X", false, chat_Windows[player].window)
chat_Windows[player].Label = guiCreateLabel(10, 263, 609, 15, "* [N/A] Estan escribiendo...", false, chat_Windows[player].window)
...
function(  )
	if ( source == GUIEditor.button[1] ) then
		clickTimer( GUIEditor.button[1], 3 )
			local Sel = guiGridListGetSelectedItem( GUIEditor.gridlist[1] )
				if ( Sel == -1 ) then outputChatBox( '#FF0000• ERROR :#FFFFFF Busca al jugador con el que quieres iniciar un chat!', 255, 255, 255, true ) return end
				local player = getPlayerFromName( guiGridListGetItemText( GUIEditor.gridlist[1], Sel, 1 ) )
					if ( player == Cplayer ) then outputChatBox( '#FF0000• ERROR :#FFFFFF Que eri weon, no puedes hablar contigo', 255, 255, 255, true ) return end
	...
	elseif ( source == GUIEditor.button[4] ) then
		...
		if ( guiGetText( GUIEditor.button[4] ) == 'Finalizar el chat' ) then
			guiSetEnabled( GUIEditor.button[1], false )
				...
				guiSetText( GUIEditor.button[4], 'Empezar un chat' )
			setElementData( Cplayer, 'chatStatus', 'Desconectado' )
			triggerServerEvent( 'onServerChangeStatus', Cplayer, 'Desconectado' )
			for cNumber, _ in pairs( chat_Windows ) do
			...
			guiSetEnabled( GUIEditor.button[1], true )
				...
				guiSetText( GUIEditor.button[4], 'Finalizar el chat' )
			setElementData( Cplayer, 'chatStatus', 'Conectado' )
			triggerServerEvent( 'onServerChangeStatus', Cplayer, 'Conectado' )
		end
	...
	elseif ( source == GUIEditor.gridlist[1] ) then
		local Sel = guiGridListGetSelectedItem( GUIEditor.gridlist[1] )
			if ( Sel == -1 ) then
				guiSetEnabled( GUIEditor.button[2], false )
				guiSetText( GUIEditor.button[2], 'Bloquear jugador' )
			else
	...
	elseif ( source == GUIEditor.button[2] ) then
		...
		local player = getPlayerFromName( guiGridListGetItemText( GUIEditor.gridlist[1], Sel, 1 ) )
			local serial = getElementData( player, 'chatSystem;playerSerial' )
		if ( guiGetText( GUIEditor.button[2] ) == 'Bloquear jugador' ) then
	...
	elseif ( source == GUIEditor.button[3] ) then
		...
		local Sel = guiGridListGetSelectedItem( GUIEditor.gridlist[1] )
			if ( Sel == -1 ) then outputChatBox( '#FF0000• ERROR :#FFFFFF Selecciona al jugador que le quieres dar un aviso!', 255, 255, 255, true ) return end
				local player = getPlayerFromName( guiGridListGetItemText( GUIEditor.gridlist[1], Sel, 1 ) )
			local serial = getElementData( player, 'chatSystem;playerSerial' )
	...
	elseif ( source == GUIEditor.combobox[1] ) then
		local Sel = guiComboBoxGetSelected( GUIEditor.combobox[1] )
			if ( Sel == -1 ) then return end
		if ( guiComboBoxGetItemText( GUIEditor.combobox[1], Sel ) == 'Conectado' ) then
				guiGridListClear( GUIEditor.gridlist[1] )
			for _, player in ipairs( getElementsByType( 'player' ) ) do
				if ( getElementData( player, 'chatStatus' ) == 'Conectado' ) then
					addOnlinePlayer( player )
				end
			end
		elseif ( guiComboBoxGetItemText( GUIEditor.combobox[1], Sel ) == 'Offline' ) then
				guiGridListClear( GUIEditor.gridlist[1] )
			for _, player in ipairs( getElementsByType( 'player' ) ) do
				if ( getElementData( player, 'chatStatus' ) == 'Desconectado' ) then
					addOfflinePlayer( player )
				end
			end
		elseif ( guiComboBoxGetItemText( GUIEditor.combobox[1], Sel ) == 'Ambos (All)' ) then
				guiGridListClear( GUIEditor.gridlist[1] )
			for _, player in ipairs( getElementsByType( 'player' ) ) do
				addPlayer( player )
			end
	...
	elseif ( source == GUIEditor.checkbox[1] ) then
		if ( guiCheckBoxGetSelected( GUIEditor.checkbox[1] ) == true ) then
			setElementData( Cplayer, 'donotDisturb', 'Activo' )
			triggerServerEvent( 'onServerChangeStatus', Cplayer, 'Conectado' )
		else
			setElementData( Cplayer, 'donotDisturb', nil )
			triggerServerEvent( 'onServerChangeStatus', Cplayer, 'Conectado' )
...
function showWriteMessage( player, name )
	if ( isTimer( writeTimer ) ) then return end
		guiSetText( chat_Windows[player].Label, '* ['..name..'] Esta escribiendo...')
...
function hideWriteMessage( player )
	...
	guiSetText( chat_Windows[player].Label, '* [N/A] Esta escribiendo...')
	guiSetVisible( chat_Windows[player].Label, false )
end;
...
function( pokedBy )
	if ( getElementData( Cplayer, 'donotDisturb' ) ~= 'Enabled' ) then
		...
		outputChatBox( '#FFFF00• En el chat :#FFFFFF este jugador  : [ '..pokedBy..' ] - Te ha dado un toque! !', 255, 255, 255, true )
	end
end );
...
function buildChattingWith(  )
	local row, column = guiGridListGetSelectedItem( GUIEditor.gridlist[1] )
		...
		local player = getPlayerFromName( guiGridListGetItemText( GUIEditor.gridlist[1], row, column ) )
			if ( getElementData( player, 'chatStatus' ) ~= 'Online' ) then
				outputChatBox( '#FF0000• ERROR :#FFFFFF Una lata, este jugador no esta conectado', 255, 255, 255, true ) return end
...
function privateChatDoubleClicks(  )
	if ( source == GUIEditor.gridlist[1] ) then
		local Sel = guiGridListGetSelectedItem( GUIEditor.gridlist[1] )
			if ( Sel == -1 ) then return end
		local player = getPlayerFromName( guiGridListGetItemText( GUIEditor.gridlist[1], Sel, 1 ) )
			if ( player == Cplayer ) then outputChatBox( '#FF0000• ERROR :#FFFFFF No seai weon, no te puedes hablar a ti mismo!', 255, 255, 255, true ) return end
			local serial = getElementData( player, 'chatSystem;playerSerial' )
		triggerServerEvent( 'onServerCheckIfBlocked', Cplayer, serial )
	end
end;
...
function( player, message )
	...
	guiMemoSetCaretIndex( chat_Windows[player].memo, string.len( oldMessages ) )
		if ( guiGetVisible( GUIEditor.window[1] ) == false ) then
			if ( getElementData( Cplayer, 'donotDisturb' ) ~= 'Enabled' ) then
				outputChatBox( '#FFFF00• Chat System :#FFFFFF El jugador : [ '..getPlayerName( player )..' ] - Te ha enviado un mensaje, respodele, puede ser un nuevo amigo xD !', 255, 255, 255, true )
