...
local classInfo = {
	...
	btn = {className = 'Button', height = 30, padding = {2, 4}},
...
function _buildWindow(wnd, baseWnd, parentWnd)
	...
	elseif wndClass == 'imgc' then
		elem = guiCreateStaticImage(wnd.x, wnd.y, wnd.w, wnd.h, wnd.src or '', wnd.a, parentWnd.element)
	...
	elseif wndClass == 'txt2' then
		elem = guiCreateEdit(relX, relY, 0.85, relHeight, wnd.text or '', true, parentWnd.element)
		if wnd.onchanged then
			addEventHandler('onClientGUIChanged', elem, wnd.onchanged)
		end
	else
	...
	if wnd.onclick then
		if wndClass == 'img' or wndClass == 'imgc' then
...
function showAllWindowsExc(wndpassed)
	for wnd,_ in pairs(g_openedWindows) do
		if wndpassed ~= wnd then
			guiSetVisible(wnd.element, true)
		end
	end
end
