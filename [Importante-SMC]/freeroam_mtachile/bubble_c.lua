...
function addBubble(text, player, tick, color)
	...
	table.insert(messages[player], {["text"] = text, ["player"] = player, ["tick"] = tick, ["endTime"] = tick + 2000, ["alpha"] = 0, ["texture"] = _texture, ["color"] = color, ["players"] = players})
end
...
addEventHandler("onChatIncome", root,
    function(message,color)
		if source ~= localPlayer then
			addBubble(message, source, getTickCount(),color)
                    ...
                    for i, v in ipairs(pMessage) do
                        if isElement(v.player) then
                            if tick-v.tick < timeVisible then
                                ...
                                if getDistanceBetweenPoints3D(x, y, z, px, py, pz) < distanceVisible and isLineOfSightClear ( x, y, z, px, py, pz, true) then
                                    ...
                                    if sx and sy then
                                        ...
                                        if bubble then
                                            dxDrawImage ( sx-width/2-10, yPos - 16, width+16, 20, v.texture, nil, nil, tocolor(v.color[1], v.color[2], v.color[3], v.alpha) )
                                        else
