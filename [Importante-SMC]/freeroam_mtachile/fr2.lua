...
function giveWeaponCommand(cmd, weapon, amount)
	...
	if isPlayerAiming(localPlayer) then errMsg ("No puedes sacar armas mientras recargas") return end
	if isPedReloadingWeapon(localPlayer) then errMsg ("No puedes sacar armas mientras recargas") return end
...
function showvipimg()
	dxDrawImage(0,0,screenW,screenH,'img/vipimg.jfif',0,0,0,tocolor(255,255,255,255),false)
...
wndMain = {
	...
	controls = {
		...
		{'imgc', id='btn8', src="img/btn8.png", x=30,y=280,w=60,h=60, a=false, onclick=function() end}, --exports.DogMod:DMCreateDog()
...
function gotoloc(leaf)
	...
	exports.antispawnkill.setT()
	closeWindow(wndLoc)
	showCursor(false)
end
--addEvent('showmapimg', true)
...
local imgloc = false
local locimgsrc = 'canal13'

function locWndShow()
	imgloc = true;
end

function locWndClose()
	imgloc = false
end

function showimg()
	--outputChatBox(tostring(imgloc))
	if imgloc then
		local x,y = guiGetPosition(wndLoc.element,false)

		if dxDrawImage ( x+320, y, 512, 512, 'locations/'..locimgsrc..'.png',0,0,0,tocolor(255,255,255,200)) then

			dxDrawLine  ( x+320, y, x+320+512, y, tocolor(0,0,0,200),2)
			dxDrawLine  ( x+320, y, x+320, 512+y, tocolor(0,0,0,200),2)
			dxDrawLine  ( x+320+512, y, x+320+512, 512+y, tocolor(0,0,0,200),2)
			dxDrawLine  ( x+320, 512+y, x+320+512, 512+y, tocolor(0,0,0,200),2)
		end
...
function changeimg(leaf)
	imgloc = true
	locimgsrc = tostring(leaf.file)
end
