...
function cdm_initialize()
	...
	createBlip(391.2119140625,-1545.6363525391,32.176696777344,6)
...
function cdm_showScreen( player )
	...
	triggerClientEvent(player,'init',player, teamRoot[last].text, last)
...
function cdm_changeSpawn( player, commandName, key )
	...
	triggerClientEvent('changeteam',player,last,teamRoot[last].text)
...
function cdm_spawn( player )
	...
	setTimer( cdm_spawnTwo, 1, 1, player, teamNum, randSpawn, randSkin, team )
...
function cdm_spawnTwo( player, teamNum, randSpawn, randSkin, team )
	...
	triggerClientEvent('selectteam',player)
end
...
function cdm_gamemodeMapStart( startedMap )
	...
	if ( cdm_error == false ) then
		...
		local players = getElementsByType( "player" )
		for k,v in ipairs(players) do
			setElementData( v, "Puntaje", 0 )
			spawnPlayer( v, 10000, 10000, 10000, 0, 0 )
			cdm_showScreen( v )
			fadeCamera ( v, true )
		end
		setTimer( call, 1000, 1, getResourceFromName("scoreboard"), "addScoreboardColumn", "Puntaje" )
	end
end

function cdm_playerJoin()
	setElementData( source, "Puntaje", 0 )
...
function cdm_playerWasted( totalAmmo, killer )
	...
	if ( killer ) then
		if ( killer ~= source ) then
			setElementData( killer, "Puntaje", getElementData( source, "Puntaje" ) + 1 )
		else
			setElementData( killer, "Puntaje", getElementData( source, "Puntaje" ) - 1 )
		end
	end
...
function sendkilldata(player)
	local pacoskills = 0
	local manifestanteskills = 0

	local pacos = getPlayersInTeam(getTeamFromName("Pacos"))
	local manifestatntes = getPlayersInTeam(getTeamFromName("Manifestantes"))

	for i,v in ipairs(pacos) do 
		pacoskills = pacoskills + getElementData(v,'Puntaje')
	end 
	
	for s,d in ipairs(manifestatntes)do 
		manifestanteskills = manifestanteskills + getElementData(d,'Puntaje')
	end 
	
	triggerClientEvent(player,'killsdata',player,pacoskills,manifestanteskills)
