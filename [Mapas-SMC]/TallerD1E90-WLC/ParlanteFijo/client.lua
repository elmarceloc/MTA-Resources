...
button = { }
window = guiCreateWindow(rx/2 - (370/2), ry/4, 373, 194, "Radio del Taller", false)
...
button["reproducir"] = guiCreateButton(20, 110, 75, 35, "Play", false, window)
guiSetFont(button["reproducir"], "default-bold-small")
...
local font0_EmblemaOne = guiCreateFont("ParlanteFijo/fonts/EmblemaOne.ttf", 10)
...
--------------------------
local isSound = false
addEvent ( "onPlayerViewSpeakerManagmentEscenario", true )
addEventHandler ( "onPlayerViewSpeakerManagmentEscenario", root, function ( current )
	...
	if ( toState == true ) then
		...
		if ( current ) then guiSetText ( CurrentSpeaker, "Parlante:" ) isSound = true
...
addEventHandler ( "onClientGUIClick", root, function ( )
	if ( source == button["close"] ) then
		guiSetVisible ( window, false ) 
		showCursor ( false )
	elseif ( source == button["reproducir"] ) then
		if ( isURL ( ) ) then
			triggerServerEvent ( "onPlayerPlaceSpeakerBoxEscenario", localPlayer, guiGetText ( url ), isPedInVehicle ( localPlayer ),1 )
			guiSetText ( CurrentSpeaker, "Parlante: Activado" )
			isSound = true
	...
	elseif ( source == button["remove"] ) then
		triggerServerEvent ( "onPlayerDestroySpeakerBoxEscenario", localPlayer )
	...
	elseif ( source == button["v-"] ) then
		if ( isSound ) then
			...
			if ( toVol > 0.0 ) then
				...
				triggerServerEvent ( "onPlayerChangeSpeakerBoxVolumeEscenario", localPlayer, toVol )
...
speakerSound = { }
addEvent ( "onPlayerStartSpeakerBoxSoundEscenario", true )
addEventHandler ( "onPlayerStartSpeakerBoxSoundEscenario", root, function ( who,x,y,z, url, isCar )
...
addEventHandler ( "onPlayerDestroySpeakerBoxEscenario", root, function ( who ) 
...
-- Volume				--
--------------------------
addEvent ( "onPlayerChangeSpeakerBoxVolumeCEscenario", true )
addEventHandler ( "onPlayerChangeSpeakerBoxVolumeCEscenario", root, function ( who, vol ) 
	if ( isElement ( speakerSound [ who ] ) ) then
