...
function sound(file,block,anim,duration,theplayer)
    if block then
		setPedAnimation(theplayer,block,anim,tonumber(duration),true,true,true,false)
    else
        exports.customanimations:setPedCustomAnimation(theplayer,anim,tonumber(duration),true,true,true,false)
    end

	triggerClientEvent("playsound",root,file,block,anim,duration,theplayer)
end
addEvent('sound',true)
