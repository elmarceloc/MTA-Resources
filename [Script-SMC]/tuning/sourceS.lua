...
function showMenu(player)
	...
	if vehicle then
		...
		local markerX, markerY, markerZ = getElementPosition(player)
		--local markerRotation = getElementData(player, "tuningMarkerSettings")[2] or 0
		
		enteredMarkerData[vehicleController] = {player, vehicle}
		...
		--setElementDimension(player, 65535)
		
		triggerClientEvent(vehicleController, "tuning->ShowMenu", vehicleController, vehicle)
	end
end

addCommandHandler('tuning',function(player)
	showMenu(player)
end
)
