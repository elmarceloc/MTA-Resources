...
function createPlaylist()
	...
	videos = playlist:AddColumn("Lista de reproducción",391)
end

function createUserlist()
	userlist = dxGrid:Create(852, 298, 391, 628)
	names = userlist:AddColumn("Visitantes",391)
end
...
function syncPlaylist(list)
	if playlist then
		...
		for i,v in pairs(list) do			
			if i == 2 then
				playlist:AddItem(videos,"A continuación:","",148,227,150)
			end
			if i == 1 then
				playlist:AddItem(videos,"Reproduciéndo:","",148,227,150)
...
function syncVotes(voteCount,maxVotes)
	if voteCount then
		...
		function displayVotes()
			if isPanelVisible then
			--	triggerEvent("notification",resourceRoot,"Error de la pantalla","F en el chat",4500)

				Text("Votos para saltar: "..voteCount.."/"..maxVotes, 1051, 207, 1192, 233, tocolor(255,255, 255, 255), 1.10, "default", "center", "center", true, false, true, false, false)
			end
		end
	end
end
...
function onCursorMove ( relativeX , relativeY , absoluteX , absoluteY ) 
	injectBrowserMouseMove ( test , absoluteX , absoluteY ) 
	injectBrowserMouseMove ( Screen , absoluteX , absoluteY ) 
end 

function mousebind(button, state)
	if state == "down" then
		injectBrowserMouseDown(test, button)
		injectBrowserMouseDown(Screen, button)
	else
		injectBrowserMouseUp(test, button)
		injectBrowserMouseUp(Screen, button)
	end
end

function onKey(button)
	if button == "mouse_wheel_down" then
		injectBrowserMouseWheel(test, -40, 0)
		injectBrowserMouseWheel(Screen, -40, 0)
...
function playSong(url,title,length,nick,atTime)
	...
	if isElementWithinColShape(localPlayer,cinemaCol) then
		...
		if Screen then
			...
			Screen = createBrowser(1280,718,false,false) --orig: 599,321  -- standard-tv 640,360  -- standard-cin 683,360
		...
		if title and tonumber(length) then
			...
			function displayTitle()
				...
				if isPanelVisible then
					Text("Reproduciendo: "..title.."\nDuración: "..remaining.." / "..length.."\nAñadido por: "..nick, 852, 170, 1243, 249, tocolor(255, 255, 255, 255), 1.05, "default", "left", "top", true, false, true, false, false)
					...
					Text("Saltar", 1197, 207, 1243, 233, tocolor(255, 255, 255, 255), 1.10, "default", "center", "center", true, false, true, false, false)
...
function playUrl(url)
	...
	--	requestBrowserDomains({'img1.ak.crunchyroll.com'})
		...
		--	injectBrowserMouseDown(Screen,'left')
			...
			Screen = createBrowser(1280,720,false,false) --orig: 599,321  -- standard-tv 640,360  -- standard-cin 683,360
			addEventHandler("onClientBrowserCreated",Screen,function()
				loadBrowserURL(Screen,url)
				addEventHandler("onClientBrowserLoadingFailed",Screen,function(url,err,errorDescription)
					if err == 3 then
					--	triggerEvent("onDxPopupMessage",rootelement,"SCREEN ERROR - ERR_ABORTED: '"..url.."'",4500)
						
						outputDebugString("SCREEN ERROR - ERR_ABORTED: '"..url.."'")
						triggerEvent("notification",resourceRoot,"No cargo la pagina","F en el chat",4500)
...
addEvent("playRequestUrl",true)
addEventHandler("playRequestUrl",resourceRoot,playUrl)
...
function toggleMuted()
	if Screen then
		...
		--	triggerEvent("onDxPopupMessage",resourceRoot,"Playback muted",2000)
			triggerEvent("notification",resourceRoot,"Sonido","Muteado",3000)
		else
			setBrowserVolume(Screen,screenVolume)
			--triggerEvent("onDxPopupMessage",resourceRoot,"Playback unmuted",2000)
			triggerEvent("notification",resourceRoot,"Sonido","Desmuteado",3000)
...
function changeScreenVolume(state)
	...
	triggerEvent("notification",resourceRoot,"Volumen",""..math.round(screenVolume*100),3000)
...
addEventHandler('notification',root,function(title,sub,dur)
	...
	exports.notifications.createNotification(1,'',title,sub,dur)
