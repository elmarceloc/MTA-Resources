...
colorPicker = {
	...
	constructor = function( info )
		...
		colorPicker.GUI.selectWindow = guiCreateStaticImage(screenW - info.selectWindow.width, (screenH - info.selectWindow.height) / 2, info.selectWindow.width, info.selectWindow.height, "gfx/rect.png", false)
		guiSetAlpha(colorPicker.GUI.selectWindow,0.8)
		guiSetVisible(colorPicker.GUI.selectWindow, false)
		colorPicker.children.labelTitle = guiCreateLabel(0, 0.01, 1, 1, "SELECIONE UMA COR", true, colorPicker.GUI.selectWindow)
		guiSetFont(colorPicker.children.labelTitle, "default-bold-small")
		guiLabelSetHorizontalAlign(colorPicker.children.labelTitle,"center")
		colorPicker.GUI.palette = guiCreateStaticImage(colorPicker.selectWindow.paletteX, colorPicker.selectWindow.paletteY,
		                                        colorPicker.selectWindow.plleteWPos, 256, "gfx/palette.png", false, colorPicker.GUI.selectWindow)
		...
		colorPicker.GUI.historyLabel = guiCreateLabel(info.selectWindow.historyX, info.selectWindow.historyY,
		                                       190, 15, "Cores usadas recentemente:", false, colorPicker.GUI.selectWindow)
											   
		colorPicker.GUI.noteLabel = guiCreateLabel(info.selectWindow.noteX, info.selectWindow.noteY,
		                                       190, 15, "Clique fora desta janela para fechar-la.", false, colorPicker.GUI.selectWindow)
		...
		checkColor3 = guiCreateCheckBox(info.selectWindow.ra3X, info.selectWindow.ra3Y,
												70, 15, "Cor 3", true, false, colorPicker.GUI.selectWindow)
		
		checkColor4 = guiCreateCheckBox(info.selectWindow.ra4X, info.selectWindow.ra4Y,
												70, 15, "Cor 4", true, false, colorPicker.GUI.selectWindow)
	...
	selectionManualInputRGB = function()
		if not colorPicker.avoidRecursion then
			colorPicker.avoidRecursion = true
			local r, g, b = 	tonumber(guiGetText(colorPicker.children.R)),
...
--This is where the color updates every time you click the palette or presets!
	...
	updateSelectedValue = function()
		...
		local luminanceX, luminanceY = paletteX + colorPicker.selectWindow.plleteWPos + colorPicker.selectWindow.luminanceOffset, paletteY
		local alphaX, alphaY = paletteX + colorPicker.selectWindow.plleteWPos + colorPicker.selectWindow.alphaOffset - 1, paletteY
--Wont work until I get the actual mouse working
		...
		if colorPicker.pickingColor then
			...
			elseif cursorX > paletteX + colorPicker.selectWindow.plleteWPos then cursorX = paletteX + colorPicker.selectWindow.plleteWPos end
			if cursorY < paletteY then cursorY = paletteY
			elseif cursorY > paletteY + 255 then cursorY = paletteY + 255 end

			setCursorPosition(cursorX, cursorY)

			colorPicker.h, colorPicker.s  = (cursorX - paletteX) / colorPicker.selectWindow.plleteWPos, (255 - cursorY + paletteY) / 255
		...
		local x = paletteX + (colorPicker.h * colorPicker.selectWindow.plleteWPos)
		...
		for i=1,3 do
		  for j=1,3 do
			...
			local y = wy + colorPicker.selectWindow.historyY + 25 + ((boxHeight + 5) * (i-1))
	...
	pickColor = function( button, state, cursorX, cursorY )
		...
		local minX, minY, maxX, maxY = wx + colorPicker.selectWindow.paletteX,
					       ...
					       wx + colorPicker.selectWindow.paletteX + colorPicker.selectWindow.plleteWPos,
		...
		minX, maxX = wx + colorPicker.selectWindow.paletteX + colorPicker.selectWindow.plleteWPos + colorPicker.selectWindow.alphaOffset,
